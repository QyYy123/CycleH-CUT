...
from .bonce import BoNCELoss


class CycleHCUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--CycleHCUT_mode', type=str, default="CycleHCUT", choices='CycleHCUT')
        parser.add_argument('--lambda_GAN', type=float, default=1.0, help='weight for GAN loss：GAN(G(X))')
        parser.add_argument('--lambda_NCE', type=float, default=1.0, help='weight for NCE loss: NCE(G(X), X)')
        parser.add_argument('--lambda_IDT', type=float, default=5, help='weight for l1 identical loss: (G(X),X)')
        parser.add_argument('--nce_idt', type=util.str2bool, nargs='?', const=True, default=False,
                            help='use NCE loss for identity mapping: NCE(G(Y), Y))')
        parser.add_argument('--nce_layers', type=str, default='1,4,8,12,16', help='compute NCE loss on which layers')
        ...
        if opt.CycleHCUT_mode == "CycleHCUT":
            parser.set_defaults(nce_idt=True, lambda_NCE=2.0)
        else:
            raise ValueError(opt.CycleHCUT_mode)
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'NCE1', 'D_B', 'G_B', 'NCE2', 'G','cycle_A','cycle_B']
        ...
        if self.isTrain:
            ...
            for nce_layer in self.nce_layers:
                self.criterionBoNCE.append(BoNCELoss(opt).to(self.device))
    ...
    def compute_G_loss(self):
        ...
        self.loss_G = (self.loss_G_A + self.loss_G_B) * 0.5 + loss_NCE_both  +  self.loss_cycle_A + self.loss_cycle_B
        return self.loss_G
    ...
    def calculate_NCE_loss1(self, src, tgt):
        ...
        feat_k_pool, sample_ids, attn_mats = self.netF1(feat_k, self.opt.num_patches, None , None)
        feat_q_pool, _ , _ = self.netF2(feat_q, self.opt.num_patches, sample_ids , attn_mats)
 
        total_nce_loss = 0.0
        for f_q, f_k, crit, nce_layer in zip(feat_q_pool, feat_k_pool, self.criterionBoNCE, self.nce_layers):
            loss = crit(f_q, f_k)
    ...
    def calculate_NCE_loss2(self, src, tgt):
        ...
        feat_k_pool, sample_ids, attn_mats = self.netF2(feat_k, self.opt.num_patches, None , None)
        feat_q_pool, _ , _ = self.netF1(feat_q, self.opt.num_patches, sample_ids , attn_mats)
