        ...
        if epoch % 5 == 0:
            ...
            if iscycle:
                ...
                for i, data_val in enumerate(dataset_val):
                    ...
                    if real_im_B.max() <= 0:
                        continue
                    L1_avg[epoch - 1, i] = abs(fake_im_B - real_im_B).mean()
                    psnr_avg[epoch - 1, i] = psnr(fake_im_B / fake_im_B.max(), real_im_B / real_im_B.max())
                    # psnr_avg[epoch - 1, i] = psnr(fake_im, real_im)
                    ssim_avg[epoch - 1, i] = ssim(fake_im_B.squeeze().squeeze(), real_im_B.squeeze().squeeze(),data_range=1)
                    nmse_avg[epoch-1, i] =nmse(fake_im_B, real_im_B)

                l1_avg_loss_B = np.mean(L1_avg[epoch - 1])
                mean_psnr_B = np.mean(psnr_avg[epoch - 1])
                std_psnr_B = np.std(psnr_avg[epoch - 1])
                mean_ssim_B = np.mean(ssim_avg[epoch - 1])
                mean_nmse_B = np.mean(nmse_avg[epoch - 1])
